# Polices embarquées (licence SIL Open Font License 1.1 — voir OFL.txt)

Toutes sous OFL (compatible GPL-3.0). Sources :

- `NotoSans.ttf` — Noto Sans (variable wdth,wght)
  https://github.com/google/fonts/raw/main/ofl/notosans/NotoSans%5Bwdth%2Cwght%5D.ttf
- `NotoSansMono.ttf` — Noto Sans Mono (variable wdth,wght)
  https://github.com/google/fonts/raw/main/ofl/notosansmono/NotoSansMono%5Bwdth%2Cwght%5D.ttf
- `NotoEmoji.ttf` — Noto Emoji, **monochrome** (variable wght)
  https://github.com/google/fonts/raw/main/ofl/notoemoji/NotoEmoji%5Bwght%5D.ttf
- `NotoSansCJK.otf` — Noto Sans CJK JP Regular (couvre CN/JP/KR)
  https://github.com/notofonts/noto-cjk/raw/main/Sans/OTF/Japanese/NotoSansCJKjp-Regular.otf
